...
def ranking(recommended_ids, userID, k, ref_time):
    ...
    ground_truth = get_ground_truth_impressions(userID, behaviors_df, ref_time)
    print("Ground truth clicked NewsIDs after ref_time:", ground_truth)
...
def run_experiments(behaviors_df, models_dict, test_user_count=10, n_dates=3, timeframe_hours=1, k=5, max_candidates=-1, filter_method="both", tfidf_vectorizer=None):
    ...
    # Randomly choose, for example, 10 users.
    ...
    # Create a "Date" column from the "Time" column (only date portion).
    behaviors_df['Date'] = behaviors_df['Time'].dt.date
    unique_dates = behaviors_df['Date'].unique()
    unique_dates = sorted(unique_dates)
    print("Unique Dates in Data:", unique_dates)

    # For example, choose 3 (n_dates) reference dates evenly spaced in the range.
    if len(unique_dates) >= n_dates:
        # Use np.linspace to select indices.
