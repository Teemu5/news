    ...
    for cluster_id in ordered_clusters:
        ...
        for i, user_id in enumerate(tqdm(user_list, desc=f"Evaluating users in cluster {cluster_id}")):
            try:
                ...
                partial_buffer = evaluate_scores(candidate_scores, "bagging", f"bagging_{partial_csv}", partial_buffer)
                for model_key, single_model in models_dict.items():
                    partial_buffer = evaluate_scores(separate_scores[model_key], model_key, f"{model_key}_{partial_csv}", partial_buffer)
